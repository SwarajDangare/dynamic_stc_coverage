...
def generate_launch_description():
    ...
    pkg_share = os.path.join(ws, 'share', 'dyno_bot')
    ...
    dynobs = TimerAction(
        period=6.0,
        actions=[Node(
            package='dyno_bot', executable='random_mover',
    ...
    stc = TimerAction(
        period=7.0,
        actions=[Node(
            package='dyno_bot', executable='stc_controller',
            name='stc_controller', output='screen'
        )]
    )

    visualizer = Node(
        package='dyno_bot', executable='coverage_visualizer',
